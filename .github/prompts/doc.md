...
```
my-skill/
├── PROMPT.md      # (Required) Metadata & instructions
...
Đây là một kỹ năng chỉ có hướng dẫn (instruction-only), chỉ cần tạo file `PROMPT.md`.

### Bước 1: Tạo thư mục

```bash
mkdir -p ~/.copilot/copilot/skills/code-review
```

### Bước 2: Tạo PROMPT.md
...
> **Lưu ý**: File `PROMPT.md` chứa siêu dữ liệu (name, description) ở trên cùng, sau đó là các chỉ dẫn. Agent sẽ chỉ đọc siêu dữ liệu và chỉ tải hướng dẫn khi cần.
...
### Bước 3: Tạo PROMPT.md

**`.github/skills/license-header-adder/PROMPT.md`**:
